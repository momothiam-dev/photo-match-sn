...
export default function CheckoutContent() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const response = await fetch(`/api/photo-data?photoId=${photoId}`)
        if (!response.ok) {
        ...
        const { photo: photoData, event: eventData } = await response.json()
        setPhoto(photoData)
        setEvent(eventData)
      } catch (err) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // Sinon, rediriger vers la page de succès interne (paiement simulé réussi)
      router.push(`/user/success?purchaseId=${purchaseId}`)
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-brand-navy text-white px-6 py-4">
        <div className="max-w-4xl mx-auto">
          <Link href={`/user/photo/${photo.id}`} className="text-white/60 hover:text-white transition-colors mb-2 block">← Retour à la photo</Link>
